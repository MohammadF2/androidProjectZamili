...
public class UpdateInfoActivity extends AppCompatActivity {
        ...
        StringRequest request = new StringRequest(Request.Method.GET, url, new Response.Listener<String>() {
            ...
            public void onResponse(String response) {
                if(!response.equals("No user found with this email.")){
                    ...
                    String lastName = usr.getL_Name();
                    String phoneNumber = usr.getPhone_number();
    ...
    private void updateUserInformation(User updatedUser) {
        ...
        if (firebaseUser != null) {
            firebaseUser.updateEmail(updatedUser.getEmail()).addOnCompleteListener(new OnCompleteListener<Void>() {
                 @Override
                public void onComplete(@NonNull Task<Void> task) {
                    if (task.isSuccessful()) {DataBase.database.collection("users").document(firebaseUser.getUid()).set(updatedUser).addOnCompleteListener(new OnCompleteListener<Void>() {
                          @Override
                          public void onComplete(@NonNull Task<Void> task) {
